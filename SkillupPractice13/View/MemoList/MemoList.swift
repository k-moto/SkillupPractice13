...
class MemoList: NSObject, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: MemoListCell.identifier,
                                                 for: indexPath) as! MemoListCell
        
        let currentData = dataList[indexPath.row]
        cell.setData(memoModel: currentData)
    ...
    func setMemoCountLabel(dataCount: Int){
        var memoCountText = "メモなし"
        if dataCount > 0 {
            memoCountText = String(dataCount) + "件のメモ"
        }
        
        memoCountLabel.title = memoCountText
